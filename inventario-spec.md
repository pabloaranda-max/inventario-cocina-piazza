# inventario.html — Especificación técnica v4.14
...
| — | **"Re-exportar" no muestra la cobertura R15** — el botón de una sesión ya exportada llama `exportarSesionInventario` con `force=true`, que salta la confirmación con cobertura (fiel al diseño R15: solo se enriquecieron los diálogos que ya existían, y ese camino nunca tuvo uno). Re-exportar una toma a medias no avisaría de los CEROs. Añadir el diálogo ahí es trivial (`force && !opts?.hashVerificado` distingue ese botón del camino "plantilla fresca", que sí confirma por su cuenta) | CANDIDATO — detectado en la auditoría 2026-07-28; decisión de Pablo, porque añade un diálogo donde hoy no hay ninguno |

...
| R11b | **Conteo desglosado — captura** — UI de desglose en inventario.html (líneas por presentación + "Abierto" en unidad base), pantalla de validación y badge. Probar en beta/staging | CÓDIGO LISTO 2026-07-23 en rama `feat/slice-r11-desglose` — smoke local `tests/smoke-r11b.py` (Playwright + Worker stubeado, 17 asserts: siembra del número previo, 3 líneas, quitar desglose con confirm, camino legacy intacto, validación con conteo de desglosados, persistencia tras reload). **Falta el Done real: probar en beta/staging antes de merge a main** |

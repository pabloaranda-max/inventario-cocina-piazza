...
// ── CONFIGURACIÓN ──────────────────────────────────────────
const SPREADSHEET_ID  = '117kjGFX9MYxYpdYwXkeX2qo3XJ_1HPHz3f3Cx0DtkEg';
...
function doGet(e) {
  ...
  return ContentService
    .createTextOutput(JSON.stringify({ ok: true, mensaje: 'Apps Script GENERAL activo ✓' }))
    .setMimeType(ContentService.MimeType.JSON);
}
...
function procesarInventario(payload) {
  ...
  const todasFilasDetalle = [
    ...filasCatalogados.map(f => [...f.slice(0, -1), '', f[f.length - 1]]),

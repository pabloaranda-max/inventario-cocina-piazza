// ============================================================
//  INVENTARIO UNIVERSAL DE HAMBURGUESAS — Apps Script Backend
//  (R8 multi-centro: UN script para todo el centro UH — la
//   columna Almacén de MAESTRA distingue UH_GENERAL/COCINA/BARRA)
//
//  INSTALACIÓN (una sola visita al editor):
//  1. clasp create/push desde scripts/universal/
//  2. Abrir el editor (clasp open) → ejecutar setup() → autorizar
//     — setup() crea el Spreadsheet y la carpeta de fotos y guarda
//     sus IDs en Script Properties (no hay que pegar nada aquí)
//  3. Implementar → Aplicación web (Ejecutar como: Yo · Acceso:
//     Cualquier persona) → copiar URL → AREA_CONFIG/ALMACENES['UH']
// ============================================================

// ── CONFIGURACIÓN ──────────────────────────────────────────
// IDs en Script Properties (los escribe setup(); nada hardcodeado)
...
/**
 * Ejecutar UNA VEZ desde el editor (dispara la autorización de permisos).
 * Crea el Spreadsheet y la carpeta de fotos si no existen aún y guarda
 * sus IDs en Script Properties. Reejecutar es inofensivo.
 */
function setup() {
  let ssId = _props.getProperty('SPREADSHEET_ID');
  if (!ssId) {
    const ss = SpreadsheetApp.create('Inventarios · Universal de Hamburguesas');
    ssId = ss.getId();
    _props.setProperty('SPREADSHEET_ID', ssId);
  }
  let folderId = _props.getProperty('DRIVE_FOLDER_ID');
  if (!folderId) {
    const folder = DriveApp.createFolder('Inventarios Universal — fotos');

...
> —venían con 47, 19, 19, 13, 6 y 0—, que era el objetivo de R25; UH quedó intacto
> y `UH_COCINA` conserva sus 2 sin nombre porque su plantilla es del 24 y no se
> recargó. **GENERAL deja de estar sin plantilla**: XTINV000285, 434 artículos. La
> asignación de los 7 archivos no se dedujo del nombre sino cruzando códigos contra
> producción: 6 dieron 100 % contra su almacén y GENERAL se resolvió contra su
> catálogo (97.7 % frente a 34.8 % del segundo candidato). **Quedó un residuo**: el
> catálogo de GENERAL conserva 230 artículos de la plantilla de ALIMENTARI
> (XTINV000288), que estuvo cargada ahí antes de corregirse. La plantilla quedó
> correcta y R25 no borra, así que el residuo es permanente hasta R27. No afecta el
> conteo —los dos consumos del catálogo filtran contra `rowMap`— y confirma que el
> hueco de R26 en la primera carga es real, no teórico.
> v4.20 (2026-07-31): una recarga de CAVA destinada a sincronizar catálogo se
> envió a `UH_MERCH`; la guarda de cruce era un confirm opcional en el cliente y
> el Worker confiaba en el destino recibido. Merch se restauró byte por byte desde
> el respaldo previo (8 artículos, XTINV000027), sin sesiones activas ni pérdida
> de conteos. Desde ahora compartir menos de 20 % de los códigos previos bloquea
> la carga tanto en Admin como en el Worker (409), sin posibilidad de override.
> La recuperación futura NO se calculará como catálogo menos plantilla: el catálogo
> también recibe artículos por `syncCatalogo` y contiene subconjuntos históricos
> legítimos. Queda especificada en R27 con procedencia por lote e imagen anterior.
...
> factores `1.0` cuando el nombre sí representa un envase (`BOTELLA DE 1 LT`) y
> sigue descartando las filas que solo repiten la unidad (`LT (1.0 LT)`). Para el
> conflicto real, una corrección operativa estrecha por código+nombre+unidad+fila
> fuente produce únicamente `BOTELLA DE 1 LT`; Admin la muestra explícitamente y
> el xlsx crudo permanece byte-idéntico para el regreso a Xetux.
...
> `tests/smoke-r15-reexport.py`, 10 asserts. Además, tras el reporte de Pablo de que
> "Cerrar con plantilla fresca" nunca le funcionó: el camino completo pasa en Chromium
> (`tests/smoke-plantilla-fresca.py`, 9 asserts, hasta las celdas del xlsx final), y la
> revisión destapó que **la guarda R16 le faltaba a esa ruta** — un export del MISMO
> almacén pasaba la validación de contados y se subía como plantilla. Cerrado con
> `senalesDeExport()` compartida por las dos rutas de subida.
> v4.14 (2026-07-28): auditoría de R13–R16 contra este spec: **sin desviaciones**, y toda
> la evidencia de pruebas citada se reprodujo verde en local. La ruta de UI de R16 quedó
> **verificada en navegador** (`tests/smoke-r16-ui.py`, 18 asserts, Chromium headless) —
> cerró el único ⚠️ que quedaba y destapó que el "guardada correctamente" de Plantillas
> se borraba antes de verse (corregido). Nuevo CANDIDATO en §15: "Re-exportar" no muestra
> la cobertura R15.
> v4.13 (2026-07-28): **R13, R14 y R15 EN PROD** — la carrera que perdía conteos dejó de
> ser un pendiente bloqueante; salieron al cerrar las tomas del 27 (Worker `5b24af4f`,
> Pages main `1b61d51`). Se suman, del día de tomas del 27: **R16 guardas de subida de
> plantilla EN PROD** (la pestaña Plantillas aceptó cuatro veces el export de CAVA como
> plantilla de SALUMERÍA), y como pendientes **R17** borrado durable, **R18** resiliencia
> de carga (nace del incidente del 07-26) y **R19** fusionar o mover tomas. Los tres
> incidentes del 26 y 27 quedaron documentados en `inventario-incidentes.md`.
...
```
R1a: factor == 1.0 y nombre == unidad base → ignorar
R1b: factor == 1.0 y nombre de envase      → conservar
...
**Corrección operativa conocida (R24).** Para `XMAT2410000317`, solo si además el
nombre contiene `UNLITRO`, la unidad es `LT` y existe la fila fuente
`XMAT2410000317_2 (1.0 LT)`, `presMap` se reemplaza por
`BOTELLA DE 1 LT × 1`. Esto neutraliza la asociación fantasma `_2283 × .75`
confirmada por Pablo el 2026-07-31. No se modifica `raw`: el archivo de retorno
conserva la estructura original de Xetux. La excepción debe retirarse cuando el
exportador de Xetux deje de emitir la relación fantasma.
...
| R6 | **Reporte de presentaciones sospechosas** — detección en admin (duplicadas, repetidas, factores sospechosos); marca y pide decisión humana, nunca autocorrige salvo una corrección operativa explícita documentada como R24 | HECHO 2026-07-08. `parsePlantilla` reporta `sospechosas[]` (factor_1, ml_g, duplicada, factor_ilegible, nombre_ambiguo) además de filtrar; admin tab Plantillas muestra "Revisión de presentaciones" calculada al vuelo desde `raw` (+ LT/KG sin presentación ni default, listas capadas a 60). Sin Worker ni migraciones. Tests parser 22/22; E2E staging 8 asserts. En prod detecta mugre real: COCINA 104×factor_1, 3×ml_g, 2×ilegible; CAVA 44×duplicada |
...
| R12 | **Guard de versión de app (anti-skew de caché)** — `APP_VERSION` en los frontends viaja en cada POST `/inv/*`; el Worker rechaza escrituras de versiones < `MIN_APP_VERSION` (426) y publica `minAppVersion` en los GET de arranque; la app bloquea al entrar y muestra banner si el sync es rechazado; imports de módulos con `?v=` para que HTML y módulo no se desfasen. Ver diseño abajo | HECHO — **EN PROD 2026-07-23** (main `70acc94`, Worker prod `6a64c4d6` con MIN=1 desplegado DESPUÉS de verificar Pages — playbook cumplido; espejo UH incluido). Staging: POST sin versión → 426, versionado → 200, GETs publican minAppVersion. Smokes: `tests/smoke-r12.py` 8 asserts (bloqueo al entrar, banner tras 426 sin perder captura local, flujo normal) + regresión R11b 17/17. Prod verificado con 6 probes estables (426 + minAppVersion:1). **R23 2026-07-31:** Piazza, admin y espejo UH v2 verificados byte a byte antes de subir prod a MIN=2; v1 → 426 y v2 atraviesa la guarda. ⚠️ Lección: probar el rechazo en prod SOLO con action inexistente (`probe_r12`) — un POST real durante la propagación del Worker puede caer en una instancia vieja y escribir (pasó; fila basura borrada por SQL) |
| R13 | **Escritura atómica de sesiones (carrera lost-update)** — el read-modify-write de `inv_sesion` no es atómico: dos dispositivos que sincronizan en la misma ventana de milisegundos se pisan y la pérdida es DEFINITIVA. Ver diseño abajo | HECHO — **EN PROD 2026-07-28** (Worker `5b24af4f`). Falla reproducida 2026-07-24; el arreglo esperó ventana con calma y salió cuando cerraron las tomas del 27. CAS sobre `updated_at` con hasta 12 reintentos y espera escalonada, sin migración ni cambio de contrato. Humo en prod: `/inv/sesiones` y `/inv/plantilla` OK, guard R12 sigue dando 426 sin `appVersion` |
| R14 | **Sync del cliente a prueba de fallos** — hoy cualquier POST fallido se descarta en silencio (`if (!r.ok) return;` + `.catch(console.warn)`); tras cerrar zona el cliente ya no reenvía conteos, así que lo capturado queda varado en el teléfono sin aviso. Ver diseño abajo | HECHO — **EN PROD 2026-07-28** (Pages, main `1b61d51`). Verificado con `tests/smoke-sync-retry.py` (15 asserts). Es lo que vuelve visible el modo de falla del incidente de CAVA del 07-26: sin banner ni reintento, una toma entera pudo quedarse en el teléfono sin que nadie lo notara |
...
| R26 | **Bloqueo autoritativo de almacén cruzado** — una plantilla que comparte menos de 20 % de los códigos previos no puede reemplazar el almacén: Admin bloquea con mensaje rojo y el Worker repite la validación con 409 | HECHO — **EN PROD 2026-07-31**. Nació de la carga accidental XTINV000289/CAVA sobre UH_MERCH durante R25; restauración exacta desde backup, sin sesiones ni pérdida de conteos. Smoke UI y Worker cubren que no existe override y que plantilla/catálogo permanecen intactos. **HUECO CONFIRMADO 2026-07-31, no cerrado:** la guarda solo corre `if (previousTemplate?.row_map)`, así que **la primera carga a un almacén sin plantilla no está protegida en ninguna de las dos capas** — por ahí entró XTINV000288 a GENERAL el mismo día. El arreglo propuesto es comparar contra `catalogo_articulos` cuando no hay plantilla previa: medido en producción, ese cruce separa GENERAL con 97.7 % contra 34.8 % del segundo candidato, así que discrimina de sobra. Otros dos filos sin decisión: no hay salida soportada para un reemplazo legítimo en almacenes chicos (UH_MERCH necesita 2 códigos en común, UH_SUMINISTROS 3) y el `catch(_){}` del `JSON.parse` deja pasar la carga si el `row_map` está corrupto |
| R27 | **Procedencia y reversión de cargas de catálogo** — registrar un lote por cada escritor (`inv_plantilla` y `syncCatalogo`), incluidos los valores anteriores de cada fila, para poder revertir exactamente solo lo introducido o sobrescrito por una carga. La operación será maestra, con vista previa por lote, y se negará si el almacén tiene una toma abierta. Nunca inferir contaminación mediante `catálogo − plantilla`: en producción esa resta incluye cientos de artículos legítimos | PENDIENTE — diseñado tras el incidente R26. **Ya tiene un caso real esperándolo:** el catálogo de GENERAL carga 230 artículos de ALIMENTARI (ver fila siguiente). No bloquea el conteo del 31 porque UH_MERCH se restauró desde respaldo y el residuo de GENERAL es invisible en operación |
| — | **Residuo de ALIMENTARI en el catálogo de GENERAL** — verificado en D1 el 2026-07-31: GENERAL tiene 434 en plantilla y 672 en catálogo; de los 238 sobrantes, **230 son códigos de XTINV000288 (ALIMENTARI)**, que estuvo cargada en GENERAL antes de corregirse a XTINV000285. R25 no borra, así que quedaron fijos. Limpiar debe dejar 442. **No usar `catálogo − plantilla` para hacerlo**: los otros 8 sobrantes son legítimos y uno de ellos también aparece en la plantilla de ALIMENTARI, o sea que por pura resta es indistinguible del residuo y necesita criterio humano — justo el argumento de R27 | PENDIENTE — sin efecto en el conteo: los dos consumos del catálogo (`inventario.html:847` y `:897`) filtran con `.filter(a => !rm \|\| a.codigo in rm)` y el acuse resuelve nombres solo de lo contado. Ojo con el `!rm \|\|`: si la plantilla no cargara, el filtro se salta y se mostrarían los 672 |
| — | **GENERAL sin plantilla en D1** | **RESUELTO 2026-07-31** — XTINV000285, 434 artículos, 0 sin nombre. Identificada contra su catálogo (97.7 %), no por el nombre del archivo |
...
| — | **`defaultPres` vacío en 10 de los 11 almacenes con plantilla** — verificado en D1 el 2026-07-28 y revisado el 2026-07-31 al entrar GENERAL: solo CAVA (`LT → botella .75`) y UH_COCINA lo tienen configurado. Vacíos: ALIMENTARI, BARRA, BARRA_AMICI, BARRA_RESTAURANTE, COCINA, GENERAL, SALUMERIA, UH_BARRA, UH_MERCH, UH_SUMINISTROS. Los demás cuentan todo en unidad base; correcto por diseño, pero conviene confirmarlo almacén por almacén antes de escalar. Ojo: subir una plantilla nueva NO pisa `default_pres` (el upsert no toca esa columna), así que un `{}` es que nunca se configuró, no que se haya borrado | PENDIENTE |
...
**Ampliación 2026-07-28 (pedida por Pablo):** los caminos con `force=true` —el botón
Re-exportar y "Generar XLSX" sobre una sesión ya exportada— también pasan por la
confirmación de cobertura; el texto en ese caso aclara "ya se había exportado". La
única exenta es "Cerrar con plantilla fresca" (`opts.hashVerificado`), que confirma
con su propia línea de cobertura. Verificado en navegador: `tests/smoke-r15-reexport.py`.
